...
import { Receipt, TrendingDown, Tag, CalendarDays, Lock } from 'lucide-react';
            ...
            {filtered.map(exp => (
              <div key={exp.id} className="bg-gray-50 border border-gray-200 rounded-2xl p-3.5 flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-3 report-no-print">
                  ...
                  {/* [Closing Integrity Amendment v1.0] A locked Expense
                      (closingId set) can no longer be deleted — the
                      Firestore rule would reject it anyway; this shows
                      why instead of a silently-broken button (the exact
