...
**Human-readable Business ID — Amendment:** `businessId` (the Firestore document ID) is not something a support agent can read off a screen or a customer can read over the phone — it's a long, opaque string. Every Business gets a second, permanent, human-readable `businessCode` at creation — following the exact pattern already proven correct for Purchase Batch numbering (8.3's `generateBatchNumber`/`getNextBatchSeq`): the fixed prefix `BPT-` plus a zero-padded global sequence, e.g. `BPT-000042`. Unlike `batchNumber` (business-scoped sequencing), `businessCode` must be **globally** sequenced across the whole platform, since it's precisely the identifier a platform operator uses to find *one* business among all of them — so its counter lives on a single `platform_worker_state`-shaped document (reusing the pattern 4.8.1 already introduced) rather than being client-derived per business, avoiding any collision risk across simultaneous signups at scale. `businessCode` is immutable once assigned — a support agent's saved reference to "BPT-000042" must never silently point somewhere else. This is the field Section 9.3's search bar looks up by, and the field Section 8.14's Dashboard now displays (see amendment below).
