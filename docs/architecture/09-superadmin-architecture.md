...
- Search/lookup by business name, Admin email, or `businessId`. This lookup itself reads `platform_aggregates` for the list view (9.2's rule) but resolves to a single-business detail view.
...
**Flow:**
1. Operator selects a business and enters a justification (required — 9.6's schema).
