...
## 47. Product Architect Authorization — Lifetime Owner Investment Total — Recorded

**Status: ✅ AUTHORIZED FOR IMPLEMENTATION (12 September 2026).** Recorded additively below, per this repository's own established signature-recording convention (`capital-inicial-retirement-implementation-authorization.md`'s "Amendment N — Pending" → "Amendment N — Recorded" pattern; this document's own §14/§31/§44 "signature is a separate, later, dated act" precedent) — **§46 above, including its own "DRAFT — AWAITING PRODUCT ARCHITECT SIGNATURE" status lines and the blank Formal Acceptance block at §46.7, is preserved completely unedited as the historical record of what was drafted and circulated for review.** This section is the actual, dated act of signature; §46.1–§46.6's governing chain, scope, no-new-write-path boundary, out-of-scope list, execution rule, and acceptance criteria are not restated, reworded, or reopened here — they govern exactly as drafted.

**Formal acceptance, recorded verbatim from the Product Architect's own authorization:**

> I, SABUSHIMIKE MASCENI, acting as Product Architect for SABUSH BPT, have reviewed the complete governance chain for OI-PA-6 — Lifetime Owner Investment Total — and the Implementation Authorization §46 in full: its scope (§46.2, limited exactly to Checkpoint 8), the confirmed no-new-write-path boundary (§46.3), its explicit out-of-scope boundaries (§46.4), the execution rule (§46.5), and the acceptance criteria AC-OI-LT-1 through AC-OI-LT-12 (§46.6) — and hereby AUTHORIZE IMPLEMENTATION.
>
> I authorize implementation of the Lifetime Owner Investment Total strictly according to: the accepted Specification §46/FR-82; the Rule 8 Assessment Addendum (Findings OI-7–OI-18, verdict `READY FOR PLAN`); the accepted Implementation Plan Amendment (Checkpoint 8); and the scope, boundaries, acceptance criteria, and exclusions contained in §46 of this Implementation Authorization.
>
> This authorization does not authorize redesign. Implementation must remain strictly within the accepted governance state, and must in particular preserve: the Lifetime Owner Investment Total as `SUM(OwnerInvestment.amount)` across all immutable `OwnerInvestment` records belonging to the relevant business, with no `date`, `createdAt`, snapshot-boundary, Contagem, or establishment-method filtering of any kind; the immutable `OwnerInvestment` records as the sole source of truth, with no mutable accumulator and no separately persisted lifetime-total field; `ownerInvestmentSinceLastSnapshot` (FR-65) remaining completely unchanged and independently calculated, never reused, renamed, or replaced to serve as the lifetime total; CAIXER (`cashPosition` and its four components) remaining completely uninvolved; `BusinessWorthSnapshot`, `measuredBusinessWorth`, and every Business Worth Evolution formula remaining unchanged, with the lifetime total never inserted into Business Worth History; the approved UI — a visible Lifetime Owner Investment Total and a collapsible history of individual `OwnerInvestment` records, both inside the existing Owner Investment card of `CashFlowView.tsx`, both reading from the same business-scoped `OwnerInvestment` source array, with no new top-level Owner Investment module; and the existing `OwnerInvestment` write path, schema, immutability, closed-period enforcement, subscription/trial gating, Owner-only authorization, and idempotency, all unchanged. No new Firestore collection, persisted aggregate, backend aggregation job, scheduled recomputation, new listener, migration, new index, or cross-business query is authorized. This authorization applies **only** to OI-PA-6 / Specification §46 / FR-82 / Checkpoint 8 — it does not authorize, imply, or begin any other Owner Investment work, any CAIXER work, or any unrelated governance item.
>
> **Product Architect:** SABUSHIMIKE MASCENI
> **Date:** 12 September 2026
> **Decision:**
...
**Governance gate — resolved.** §46's own Governance Gate (its closing paragraph) and its §7/§23/§36/§43-mirrored execution discipline now apply as an active authorization, not a pending one: this signature is the governance authorization gate for Checkpoint 8, named at §46.2 — it authorizes Checkpoint 8 to be implemented, subject in full to §46.3 (no-new-write-path boundary, unweakened), §46.4 (explicit exclusions, unweakened), and §46.6 (acceptance criteria AC-OI-LT-1 through AC-OI-LT-12, unweakened). None of §46.1–§46.6 is reopened, reworded, or reinterpreted by this signature.

**This signature does not itself instruct implementation of Checkpoint 8 to begin.** Per §46.5's own execution-rule discipline and this document's identical §36/§44 precedent ("no item becomes authorized to begin merely because the section is signed"), a further, separate, explicit instruction to begin Checkpoint 8 remains required before any application code, test, `firestore.rules`, or i18n file may be created or modified. **No such instruction is given, implied, or begun by this section.** No Execution Record (mirroring §37–§42's format) exists yet for Checkpoint 8.

**Next Governance Step:** a separate, subsequent, explicit instruction to begin Checkpoint 8 — not performed, drafted, or implied by this signature.
