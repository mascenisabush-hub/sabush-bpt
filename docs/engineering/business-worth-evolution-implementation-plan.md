...
## Product Architect Acceptance of Implementation Plan Amendment — Checkpoint 2 Scope Clarification (Engineering-Landing vs. Release; AppContext Draft-Persistence Scope; Voltar Test-Contract Integrity)

**Type:** Implementation Plan Amendment — narrowly scoped to three governance/quality-boundary questions surfaced by a read-only pre-commit audit of the (still uncommitted) Checkpoint 2 implementation. Not a Specification Amendment, not a Decision, not a BDR, not a Policy — decides no new business rule, introduces no new product behavior, and reopens no accepted decision. Amends only this Implementation Plan's own governing boundaries for Checkpoint 2; does not modify, reinterpret, or reopen the accepted CAIXER Specification (§45), the Rule 8 Assessment Addendum, the CX-1/CX-2/CX-6/CX-13/CX-14 acceptance, BDR Decision 40, or Implementation Authorization §44's own scope (§43.2's seven-item enumeration is clarified in its application, not expanded, narrowed, or amended).

**Status:** ✅ Accepted (11 September 2026).

**Governing basis:** this Plan's own CAIXER amendment (above); Implementation Authorization §44 (`business-worth-evolution-implementation-authorization.md`, ✅ AUTHORIZED FOR IMPLEMENTATION, §43.2/§43.5); Checkpoint 1 (commit `1738efc7e7e0b604f948f8f69e8c17cc54b9cf70`); the read-only Checkpoint 2 pre-commit governance audit that identified the three items this amendment resolves. Checkpoint 2 itself remains uncommitted as of this amendment — nothing here authorizes committing it; this amendment only resolves the scope questions the audit raised so that a subsequent, separate act may do so.

**Amendment content, in full (the only substantive change this amendment makes to the Plan):**

1. **Engineering landing vs. release (resolves Audit §1).** Checkpoint 2 may be implemented and committed as an engineering increment before Checkpoint 3 — the Plan's own §7's one-checkpoint-at-a-time execution rule (Authorization §43.5) governs *implementation sequencing* only. This is now made explicit as a distinct question from *production release/readiness*: Checkpoint 2 **must not** be considered independently releasable/deployable as a completed CAIXER feature, and the mandatory CAIXER UI/state it introduces must not be exposed to production users in a way that requires CAIXER completion while Checkpoint 3's authoritative write/calculation integration is absent. The repository may hold the incremental Checkpoint 2 implementation while the full CAIXER feature remains incomplete and unreleased.
2. **`AppContext.tsx` draft-persistence scope (resolves Audit §2).** The `AppContext.tsx` changes required to persist `PeriodicStockDraft.caixerDraft` through the **existing** periodic-draft autosave path — extending `savePeriodicStockDraftMeta`, `flushPeriodicStockDraftRows`, and `buildPeriodicDraftMeta` as necessary to carry `caixerDraft` — are explicitly confirmed as within Checkpoint 2 (Plan §C.3's own "writing back to `caixerDraft`... via the existing debounced-autosave path" requirement). This does **not** move any confirmed-write-path work into Checkpoint 2: `RecordStockCountParams`'s CAIXER write payload, `recordStockCount` itself, the authoritative confirmed-snapshot CAIXER fields, aggregate validation at the confirmed write boundary, and final Business Worth calculation integration all remain exactly where Authorization §43.2 item 3 (Plan §D/C.7–C.8) already places them — Checkpoint 3. No other `AppContext.tsx` behavior is authorized by this item.
3. **Voltar test-contract integrity (resolves Audit §3).** The two Voltar-related test modifications identified by the audit (`periodic-contagem-shared-live-data-decisions-44-56.test.ts`, `periodic-contagem-validar-decision-40.test.ts`) must continue to verify, at minimum, that Voltar: (a) clears the pending tally; (b) does so unconditionally; (c) is not gated by unrelated ownership/validation state; (d) introduces no destructive clearing of existing count/draft data — while permitting the newly-authorized `setCaixerStage(null)` effect alongside `setPendingTally(null)`. Where the current regex structure no longer structurally proves the original contract (the audit's own finding: a single-expression anchor previously proved "no other effect," which a multi-statement block no longer does by construction), the assertion must be strengthened or restructured to re-establish an equivalent guarantee — never left weakened merely for convenience. This is recorded as a binding requirement on the still-pending test correction; it does not itself perform that correction, and no test file is modified by this amendment.

> I have reviewed the Checkpoint 2 pre-commit governance audit's three findings and accept the clarifications above, exactly as stated. I confirm: (1) implementation/checkpoint landing order is distinct from production release/readiness, and Checkpoint 2 may be committed as an engineering increment without being releasable on its own; (2) the `AppContext.tsx` draft-persistence plumbing for `caixerDraft` (`savePeriodicStockDraftMeta`/`flushPeriodicStockDraftRows`/`buildPeriodicDraftMeta`) is within Checkpoint 2's authorized scope, while the confirmed write path remains Checkpoint 3, unchanged; (3) the two Voltar test assertions must preserve their original behavioral contract in full and must be strengthened, not left weakened, before Checkpoint 2 is committed. This clarification does not authorize Checkpoint 3, does not authorize production release of an incomplete CAIXER flow, and does not itself authorize committing the current Checkpoint 2 implementation — the test correction in item 3 remains outstanding, and committing Checkpoint 2 remains a separate, subsequent act.
>
> **Product Architect:** SABUSHIMIKE MASCENI
> **Decision:** ACCEPTED
> **Date:** 11 September 2026
...
Owner Investment must respect the existing closed-period mechanism (`isDateInsideClosedPeriod`/`findClosedPeriodConflict`, already governing `expenses` and `withdrawals`). An Owner Investment cannot be recorded with its business/economic `date` inside a closed period. Reuse the existing mechanism; do not create a new one. Enforcement must exist at the authoritative server/write boundary (`firestore.rules`), not only client-side UI validation. **Implemented (commit `2148793`)** — `addOwnerInvestment` (`AppContext.tsx`) now calls `findClosedPeriodConflict`, and the `ownerInvestments` create rule (`firestore.rules`) now calls `isDateInsideClosedPeriod`, both identical to the mechanism already governing `expenses`/`withdrawals`.

### OI-PA-2 — Subscription/Trial Entitlement

Owner Investment is a governed tenant feature and must respect the existing subscription/trial entitlement mechanism (`subscriptionAllowsNewRecords`/`subscriptionBlocksNewRecords`). Reuse the existing gate; do not create a new entitlement system. **Implemented at the authoritative server boundary** — the `ownerInvestments` create rule (`firestore.rules`) now calls `subscriptionAllowsNewRecords`, identical to the mechanism already governing `expenses`/`withdrawals`, folding this collection into Module #19 Phase 2's existing Restricted-Operations Enforcement (Business Rule 6 / Decision 2), whose original enumeration simply predated this collection's existence. **`subscriptionBlocksNewRecords` (the client-side UI half) is not yet wired anywhere** — that mechanism lives entirely inside the entry-point screen itself (`AddWithdrawalView.tsx`/`AddExpenseView.tsx` each check it directly), and Owner Investment has no entry-point screen yet (OI-PA-3, not yet implemented). This is a structural consequence of where the client-side gate lives, not a remaining gap in this decision's own server-side scope — it will be wired in naturally when OI-PA-3 is built, following the identical pattern.
...
## Product Architect Acceptance of Implementation Plan Amendment — Lifetime Owner Investment Total (Checkpoint 8) — Recorded

**Status: ✅ ACCEPTED (12 September 2026).** Recorded additively below, per this repository's established signature-recording convention (Specification §44.5→§44.6's own precedent; this Plan's own Checkpoint-2-Scope-Clarification precedent, above). The drafted amendment immediately above — including its own "DRAFT — AWAITING PRODUCT ARCHITECT REVIEW" status line and its own blank "Product Architect Acceptance — Pending" signature block — is preserved unedited as the historical record of what was circulated for review; this section is the actual, dated act of acceptance, covering that amendment in its entirety.

> I, SABUSHIMIKE MASCENI, acting as Product Architect for SABUSH BPT, have reviewed the Implementation Plan Amendment — Lifetime Owner Investment Total (OI-PA-6 / Specification §46, FR-82; Rule 8 Assessment Addendum, Findings OI-7–OI-18) — recorded immediately above, including Checkpoint 8's full scope (the pure derived calculation function, item 1; the Cash Flow UI total/collapsible-history addition, item 2; i18n, item 3; the testing plan, item 4), the restated economic boundaries (A–E), the protected-boundaries regression list, the Dependencies, the Explicitly Out of Scope list, and the Traceability table — and I hereby **ACCEPT it exactly as drafted, with no modification to its scope**.
>
> This acceptance confirms: the accepted implementation scope is a pure derived lifetime Owner Investment aggregation — `SUM(OwnerInvestment.amount)` across all immutable `OwnerInvestment` records belonging to the relevant business, with no filtering by `date`, `createdAt`, snapshot boundary, Contagem, or Owner-Declared establishment method; no mutable or persisted accumulator; no new Firestore collection; no backend aggregation job; no new listener; no migration; no new index. FR-65 (`ownerInvestmentSinceLastSnapshot`), CAIXER, and Business Worth History all remain completely separate and unmodified. The existing `OwnerInvestment` write behavior, closed-period enforcement, subscription/trial gating, Owner-only authorization, and idempotency all remain unchanged. The approved UI scope — a visible Lifetime Owner Investment Total and a collapsible history of individual `OwnerInvestment` records, both inside the existing Owner Investment section of `CashFlowView.tsx`, both reading from the same business-scoped `OwnerInvestment` source array, with no new top-level Owner Investment module and no placement inside Business Worth History — is likewise accepted exactly as drafted.
>
> **This acceptance is Plan-level only. It does NOT constitute, authorize, or imply an Implementation Authorization.** Checkpoint 8 remains gated behind a separate, subsequent, signed Implementation Authorization item in `business-worth-evolution-implementation-authorization.md`, naming Checkpoint 8 explicitly, per that document's own established one-item-at-a-time execution rule. No code, test, `firestore.rules`, `firestore.indexes.json`, or i18n file may be created or modified as a result of this acceptance. Checkpoint 8 has not begun and does not begin as a result of this acceptance.
>
> **Product Architect:** SABUSHIMIKE MASCENI
> **Date:** 12 September 2026
> **Decision:** ✅ ACCEPTED

**Next Governance Step, restated (unchanged by this acceptance):** a separate, subsequent, signed Implementation Authorization item — not created, drafted, implied, or authorized by this acceptance.
