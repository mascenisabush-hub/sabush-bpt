...
| 4 | [Purchase Batches](./04-purchase-batches.md) | ✅ Approved — **Version 1.1**, amended by the [Durable Multi-Product Purchase Capture and Reusable Suppliers Amendment](./04-durable-purchase-capture-and-suppliers-amendment.md) (✅ Approved — **implemented**, `9333689`; a production Firestore undefined-field bug in this feature was found and fixed, `0c71631`). Covers a durable, interruption-resilient purchase draft and a reusable, tenant-scoped Supplier entity. Also amended by the [Multi-Supplier Purchase Event Amendment](./04-multi-supplier-purchase-event-amendment.md) (✅ Approved — **implemented, tested, and closed**; all 7 phases complete — `f27d1ce` through `2013573`). Adds an optional `purchaseEventId` correlation across `PurchaseBatch` records for a broader restocking trip spanning several suppliers — `PurchaseBatch`'s own definition (one supplier's delivery) is unchanged by this. Explicitly excludes payment/cash/credit/supplier-debt capability, which remains a separate, not-yet-started governance track (see the first amendment's Part 11). |
...
| 18 | [SuperAdmin](./18-superadmin.md) | ✅ Accepted (documentation & business rules; implementation not authorized) — **except** two separately-authorized slices, both implemented and deployed: the [Payment Operations V1 Launch Slice](./18-19-payment-operations-slice.md) ([ADR-0005](../adr/ADR-0005-superadmin-payment-operations-boundary.md)) and the [SuperAdmin V1 Operational Control Plane](./18-superadmin-v1-operational-control-plane-slice.md) ([ADR-0006](../adr/ADR-0006-superadmin-v1-operational-control-plane.md)) — four phases (Internal Account Management, Business Visibility, Business Suspend/Reactivate, Audit Center Filtering), all implemented, tested, `firestore.rules`/`firestore.indexes.json` deployed and verified against a real Firestore engine, and production-verified in a real browser against real data. A related capability, the [Business Directory (Phase E)](./BDR-0010-superadmin-business-directory.md) ([BDR-0010](./BDR-0010-superadmin-business-directory.md), [POL-18-001](./18-pol-001-operational-activity-state-model.md)) — not counted among the four ADR-0006 phases above, governed separately — is also implemented, tested (81 non-emulator + 18/18 emulator tests), and production-verified; unlike the four phases above, its implementation began before the required Rule 8/Authorization gates completed, which is retrospectively acknowledged and accepted, not concealed — see [`18-superadmin-business-directory-retrospective-acceptance.md`](../engineering/18-superadmin-business-directory-retrospective-acceptance.md) and [`18-superadmin-business-directory-closeout.md`](../engineering/18-superadmin-business-directory-closeout.md) for the full record. See `HANDOFF.md`'s "Right now" section for the exact commit trail. The rest of Module #18 (Tenant Management, §9.4 subscription overrides, Feature Flags, Platform Analytics, platform Notifications, Impersonation, System Health, the full §9.12 Internal Account Management UI beyond what the Operational Control Plane's narrower slice already covers) remains not authorized. |
