...
1. **What exactly should be remembered — ACCEPT: general, possibly multiple alternative names, with supplier-aware memory as the priority/primary form.** BPT may remember more than one alternative name for a given product. The priority/primary form is supplier-aware: BPT should remember the wording used by each supplier for a product and associate that supplier-specific wording with the same underlying product identity. When a given supplier/wording combination is encountered for the first time, the owner may be asked to confirm whether it refers to the already-known product; only after explicit confirmation is that relationship remembered for future occurrences of the same supplier/wording combination — never established silently. Other, non-supplier wordings may subsequently also be confirmed and remembered the same way. This resolution is scoped to this business concept only. It does not resolve item 3's detailed confirmation mechanism, item 4's handling of multiple possible matches, item 5's handling of one name claimed by two products, or item 7's lifecycle/correction question — all of which remain exactly as open as before this decision, below. It does not decide any data model, schema, matching algorithm, AI/OCR provider, confidence threshold, or UI.
2. **Supplier-specific or general — PARTIALLY RESOLVED.** The priority/primary form is supplier-specific: each supplier's own wording for a product is remembered separately and associated with the same underlying product identity, so multiple suppliers may have different remembered names for the same product (item 1, above). Item 1 also permits general, non-supplier-tied alternative names as a further possibility, but how a general alternative name would relate to, coexist with, be reused across, or be distinguished from a supplier-specific one remains OPEN.
