...
**Amended by:** the Product Architect's ten-decision review, incorporated directly into this document (not filed as a separate amendment artifact, per the review's own instruction to integrate rather than append) — §6, §8, §9, §10, §14, §18, §22, §25–§26, §28, §30, §31, and §36 carry the resulting changes; sections not named here are unchanged from the original draft. Ten previously-flagged Rule 8 technical questions are resolved by this review; the remaining, genuinely lower-level questions are preserved for Rule 8 exactly as before, per the review's own explicit instruction not to fabricate resolutions beyond what was decided (§30a, §36).
...
**In scope, as one consolidated whole** (source BDR Decision 35; POL-0010 CPR-5): the Business Worth lifecycle and its five states (§6); Current and Estimated Business Worth (§7, §9); Business Worth history and snapshot data model (§8); the Cash Ledger (§10); Receivables (§11); Supplier Payables (§12); Startup Investment (§13); Contagem as measurement and reconciliation, its multi-unit valuation modes, and its boundary against +Stock (§14–§17); Embedded Profit's interaction with the new model (§18); Fecho's flexible date range (§19); Levantamentos and Quebras' unchanged participation (§20–§21); the reconciliation signal (§22); Contagem autosave and confirmation safety (§23–§24); the Owner 3-hour correction window and SuperAdmin 72-hour recovery (§25–§26); immutability (§27); historical transition (§28... see renumbering note below); the Dashboard's conceptual-only change (§29... renumbering note below); and the consolidated data model, security, auditability, and idempotency requirements this implies (§30–§34).

*(Numbering note: to keep this Specification's section numbers stable as it was drafted, the two items just named — Historical Transition and Dashboard — are Specification §31 and §32 respectively, not §28/§29; §28 is the Traceability Matrix and §29 is Acceptance Criteria, matching this document's actual section order below. This note exists solely to avoid an internal cross-reference error; it is not itself a rule.)*

**Out of scope, per the source BDR/POL-0010's own explicit boundary (Architecture Boundary Override, ARCH-1/ARCH-2):** point-of-sale functionality, checkout, invoicing, payroll, full accounting, ERP functionality, or general customer transaction management. **Also out of scope, per this Specification's own discipline (§33, Explicit Non-Goals):** any Firestore security-rules text, any UI/component/layout design, any database transaction implementation, any API design, any migration/backfill script, and any rounding/precision rule beyond the existing `POL-0001`/`POL-0002` convention — each reserved for Rule 8 Assessment/Implementation Authorization.

## 4. Terminology — Preserved, Not Redefined

**Contagem**, **Capital Inicial**, **Levantamentos**, **Quebras**, **Fecho**, **Embedded Profit**, **Business Worth**, **Current Business Worth**, **Estimated Business Worth**, **Startup Investment**, **Receivables**, **Supplier obligations/Payables**, **Cash Ledger**, and **reconciliation signal** all carry exactly the meaning the source BDR §2 and POL-0010 §4 already fix. This Specification introduces no new business term. Where a new *technical* term is needed (e.g. a specific record-type name), it is introduced explicitly in §30 and marked as a Specification-level naming choice, not a business concept.

## 5. Conflict-of-Reuse Principle Governing Every Section Below

Per the source BDR §6 item 6 and this task's own Technical Design Discipline: where an existing mechanism already does substantially the required job (Contagem draft persistence, Void & Redo's Authorization-artifact shape, the existing free-form Closing date fields), this Specification reuses and extends that mechanism rather than inventing a parallel one. Every section below states explicitly whether it is (A) reusing an existing mechanism unchanged, (B) extending an existing mechanism, or (C) introducing a genuinely new one — this labeling is carried into §30's consolidated data model and §33's non-goals.
...
Startup Investment covers everything the Owner spends establishing the business, from creation until the first confirmed Contagem (source BDR Decision 20; POL-0010 SI-1). To satisfy "must not duplicate a stock purchase already recorded via +Stock" (SI-2):

- **Stock spending** already recorded as a `PurchaseBatch`/`StockBatch` during the pre-Contagem period is **referenced**, not re-recorded — Startup Investment reporting queries existing purchase records scoped to `[businessCreatedAt, firstContagemConfirmedAt)`, the same non-duplication discipline `04-purchase-batches.md` already applies to its own aggregation ("this module only aggregates and formats").
- **Expense spending** in the same window is referenced from existing `Expense` records identically.
- **Spending with no existing record type** — labor, wages, licenses, preparation/renovation costs not already captured as a Product/Stock/Expense — needs a narrow new record:
...
**FR-16.** Startup Investment reporting must aggregate existing `PurchaseBatch`/`Expense` records by reference and date-window, never by re-recording their amounts into a duplicate transaction.
...
This was previously listed as a genuine open question (this document's prior draft, §34 item 7); it is resolved as of this review and is no longer listed among this Specification's remaining open items (§36).
...
| 27 (Dashboard) | DASH-1 | §32 | FR-4, FR-47 |
...
1. Business Worth is never shown as measured for a business with no confirmed Contagem under this model (FR-1, FR-2).
...
| 2 — New-model Contagem recovery exclusivity | 31–32, §5 item 6 | REC-1, REC-2, REC-3 | §25, §26 | AC 17 |
...
| 10 — `BusinessWorthSnapshot` vs. `StockCount` separation | 4, 5, 33 | BW-3, BW-4, SYS-10, REC-4 | §8 | AC 2, 3, 27 |
