...
At the time this BDR was originally filed, none of the following was answered by this document. Following formal Product Architect review and explicit ratification (recorded in §5.A's Resolution Log below), a subset of the §5.A items has since been resolved, and the remaining four items have themselves been explicitly ratified as REMAIN OPEN — a recorded decision that the question stays open, distinct from the item simply never having been addressed. Every remaining item — including the retained open sub-question within item 6, and all of §5.B — remains genuinely undecided; none should be inferred as answered beyond what the Resolution Log explicitly states.
...
4. **Purchase-unit level restriction — REMAINS OPEN (ratified).** The only evidence available is a single illustrative worked example (§3), which is not a business rule. The Product Architect has explicitly ratified that neither top-level-only purchasing nor purchasing at any configured level is assumed, and that this question is carried forward pending real receipt/business evidence — not merely unaddressed.
...
6. **Incomplete product configuration — PARTIALLY RESOLVED.** Silent fallback to unconverted behavior is ruled out, for consistency with the "never resolve ambiguity silently" pattern already established by Decisions 10–12 — this remains resolved and is not reopened. Whether an incomplete configuration should block entry entirely or allow entry with a warning **REMAINS OPEN (ratified)** — the Product Architect has explicitly ratified that no choice is made between blocking and warning at this time, and that this sub-question is carried forward as a formally open decision, not merely unaddressed.
...
**§5.A Resolution Log** — Decisions on items 1, 2, 5, 6 (silent-fallback prohibition only), and 8 above were made via formal Product Architect review and explicit ratification in this project's governance thread. Items 3, 4, 6 (block-vs-warn sub-question), and 7 have since also been explicitly ratified by the Product Architect — each as REMAIN OPEN — a recorded decision that the question stays open pending further evidence or business input, not merely an unaddressed item. No substantive resolution of items 3, 4, 6's block-vs-warn sub-question, or 7 has occurred; all four remain live, carried-forward open questions of this BDR.
