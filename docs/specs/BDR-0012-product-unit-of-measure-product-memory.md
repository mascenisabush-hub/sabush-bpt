...
17. **When a product is entered for the first time in a business's catalog, BPT may propose a likely unit-of-measure structure for that product — "Recognition" — under the same AI-suggestion boundary Decision 12 already establishes.** A Recognition proposal reflects general knowledge about how a product is commonly packaged, not a verified fact about this specific business, and is never authoritative on its own: the owner reviews it, may edit or correct it, and must explicitly confirm it before it becomes this business's own Product Memory for that product, consistent with Decision 13 — and even then it remains this business's own confirmed configuration, not a universal truth about the product (§3). *(Illustrative only, not a hard-coded rule: a proposal might suggest, for example, "Emb" for one product or a multi-level chain such as "Cx → Emb → Un" for another — the specific structure proposed in any real case is not fixed by this decision.)* When a product already has confirmed Product Memory, BPT retrieves and reuses that existing configuration rather than running Recognition again — Recognition applies only to a genuinely first-time entry for this business, never to a product this business has already confirmed. Recognition may never silently overwrite, modify, or replace existing Product Memory; any change to an already-confirmed configuration remains the explicit owner action Decision 14 already governs, not an automatic consequence of a new Recognition attempt. This decision authorizes Recognition as a business capability only, operating within §5.A Item 2's already-resolved single-chain scope — it does not resolve §5.A Item 3, and does not weaken §5.A Item 6's silent-fallback prohibition or its warn-and-allow-entry treatment, both of which continue to govern an unconfirmed Recognition proposal exactly as they would any other incomplete configuration.
...
At the time this BDR was originally filed, none of the following was answered by this document. Following formal Product Architect review and explicit ratification (recorded in §5.A's Resolution Log below), items 1, 2, 5, and 8 are resolved, and items 4 and 6 have been explicitly accepted/resolved. Item 3 has since also been explicitly accepted (single confirmed relationship family per product). Item 7 remains explicitly ratified as REMAIN OPEN — a recorded decision that the question stays open, distinct from the item simply never having been addressed. Item 7 and all of §5.B remain genuinely undecided; none should be inferred as answered beyond what the Resolution Log explicitly states.
...
3. **Multiple independent unit-relationship families — ACCEPT: single confirmed relationship family per product.** Each product has one active, confirmed unit-relationship family — a single strictly-ordered chain, consistent with the already-resolved Item 2 — which may contain one or multiple related units. Multiple independent, non-convertible families for one product are not required or supported in current scope. This is not a permanent prohibition: consistent with Item 2's own "scope-as-evidenced, not a permanent ceiling" framing, this decision may be reopened through governance if a genuine real-world business scenario later demonstrates the need. No second-family model, family selector, family identifier, or family-resolution mechanism is introduced or implied by this decision.
4. **Purchase-Unit Default and Receipt-Unit Flexibility — ACCEPT.** A product's confirmed top-level configured unit serves as the default purchase-entry unit. This is a default, not a restriction: a purchase may be recorded in any unit that is part of the product's own confirmed unit relationship — not merely any unit that happens to appear on a receipt — specifically so a purchase can be recorded exactly as stated on a real receipt without forcing the owner to artificially convert it into the top-level unit (e.g., recording `12 Un` directly rather than `0.5 Cx`, where `Un` is already a configured unit of that product). Once recorded, the quantity is interpreted through the confirmed relationship for combination and valuation, consistent with Decision 1. A receipt-stated unit that is not part of the product's confirmed relationship is a distinct situation, not separately decided here — it is instead governed by item 6's now-resolved warn-and-allow-entry treatment, consistent with the same non-blocking approach.
...
6. **Incomplete product configuration — RESOLVED.** Silent fallback to unconverted behavior is ruled out, for consistency with the "never resolve ambiguity silently" pattern already established by Decisions 10–12. When a quantity cannot be fully interpreted — because the product's unit-relationship configuration is incomplete, or because a stated unit cannot be related to the confirmed configuration (see item 4, above) — the system warns the owner rather than blocking entry outright, per explicit Product Architect ratification (ACCEPT: warn, allow entry). This does not reopen or weaken the silent-fallback prohibition: the system never silently proceeds as if the quantity were fully understood, and never invents a conversion it cannot support. The unresolved or unconvertible situation is clearly and specifically surfaced to the owner at the point of entry — not a generic error, and not a state the owner could mistake for a successful, fully-interpreted entry — but the owner is not prevented from completing the entry once warned.
7. **Historical-data posture — REMAINS OPEN (ratified).** Discovery Report §12–13 confirm both that real production data cannot be inspected from this environment and that historical mixed-unit prevalence is genuinely unmeasured — not classifiable as low/moderate/high risk. The Product Architect has explicitly ratified that neither a conservative-progression posture nor a data-access-mechanism-as-prerequisite posture is assumed or required, and that this risk-tolerance decision is carried forward explicitly, not merely unaddressed.
...
**§5.A Resolution Log** — Decisions on items 1, 2, 5, and 8 were made via formal Product Architect review and explicit ratification in this project's governance thread. Item 7 has since also been explicitly ratified by the Product Architect as REMAIN OPEN — a recorded decision that the question stays open pending further evidence or business input, not merely an unaddressed item. Item 3 (Multiple independent unit-relationship families), item 4 (Purchase-Unit Default and Receipt-Unit Flexibility), and item 6 (Incomplete product configuration)'s remaining block-vs-warn sub-question have each since been explicitly ratified as ACCEPT — item 3 as single confirmed relationship family per product, item 4 as previously recorded, and item 6 as warn, allow entry — all now fully resolved decisions of this BDR, not merely recommendations. No substantive resolution of item 7 has occurred; it remains the sole live, carried-forward open question of §5.A.
...
**Current status:** BDR approval has occurred (see Status, above). §5.A is now substantially resolved — items 1, 2, 5, and 8 are resolved; item 4 (Purchase-Unit Default and Receipt-Unit Flexibility) is resolved as ACCEPT; item 6 (Incomplete product configuration) is fully resolved (silent-fallback prohibited; block-vs-warn resolved as ACCEPT: warn, allow entry); item 3 (Multiple independent unit-relationship families) is resolved as ACCEPT: single confirmed relationship family per product — all per the Resolution Log in §5.A. Item 7 has been explicitly ratified by the Product Architect as REMAIN OPEN — a recorded decision to carry the question forward unresolved, not merely an unaddressed item. §5.B items 1, 2, 4, and 6 have been drafted and approved as `POL-0001` through `POL-0004`; §5.B items 3 and 5 remain undrafted, pending §5.A. No reconciliation/amendment of `BDR-0009` or `04-smart-stock-entry-amendment.md`, technical architecture, Specification, Rule 8 Assessment, or Implementation Authorization has occurred.
